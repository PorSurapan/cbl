function check()
{
    var question1 = document.quiz.question1.value;
    var question2 = document.quiz.question2.value;
    var question3 = document.quiz.question3.value;
    var question4 = document.quiz.question4.value;
    var question5 = document.quiz.question5.value;
    var question6 = document.quiz.question6.value;
    var question7 = document.quiz.question7.value;
    var question8 = document.quiz.question8.value;
    var question9 = document.quiz.question9.value;

    var correct1 = 0;
    var correct2 = 0;
    var correct3 = 0;
    var correct4 = 0;
    var correct5 = 0;
    var correct6 = 0;
    var correct7 = 0;
    var correct8 = 0;
    var correct9 = 0;

    // Question 1
    if(question1 == "localhost") {
        str = "<font size='3' color='green'>ถูกต้อง</font>";
        document.getElementById("checkQ1").innerHTML = str;

        document.getElementById("textBox1").style.color = "green";
        correct1 = 2;
    } else {
        str = "<font size='3' color='red'>ไม่ถูกต้อง</font>";
        document.getElementById("checkQ1").innerHTML = str;

        document.getElementById("textBox1").style.color = "red";
    }

    // Question 2
    if(question2 == "studyphp") {
        str = "<font size='3' color='green'>ถูกต้อง</font>";
        document.getElementById("checkQ2").innerHTML = str;

        document.getElementById("textBox2").style.color = "green";
        correct2 = 2;
    } else {
        str = "<font size='3' color='red'>ไม่ถูกต้อง</font>";
        document.getElementById("checkQ2").innerHTML = str;

        document.getElementById("textBox2").style.color = "red";
    }

    // Question 3
    if(question3 == "SET NAMES UTF8") {
        str = "<font size='3' color='green'>ถูกต้อง</font>";
        document.getElementById("checkQ3").innerHTML = str;

        document.getElementById("textBox3").style.color = "green";
        correct3 = 2;
    } else {
        str = "<font size='3' color='red'>ไม่ถูกต้อง</font>";
        document.getElementById("checkQ3").innerHTML = str;

        document.getElementById("textBox3").style.color = "red";
    }

    // Question 4
    if(question4 == "close") {
        str = "<font size='3' color='green'>ถูกต้อง</font>";
        document.getElementById("checkQ4").innerHTML = str;

        document.getElementById("textBox4").style.color = "green";
        correct4 = 2;
    } else {
        str = "<font size='3' color='red'>ไม่ถูกต้อง</font>";
        document.getElementById("checkQ4").innerHTML = str;

        document.getElementById("textBox4").style.color = "red";
    }

    // Question 5
    if(question5 == "utf8") {
    str = "<font size='3' color='green'>ถูกต้อง</font>";
    document.getElementById("checkQ5").innerHTML = str;
    correct5 = 2;
    } else {
        str = "<font size='3' color='red'>ไม่ถูกต้อง</font>";
        document.getElementById("checkQ5").innerHTML = str;
    }

    //  Question 6
    if(question6 == "root") {
    str = "<font size='3' color='green'>ถูกต้อง</font>";
    document.getElementById("checkQ6").innerHTML = str;
    correct6 = 2;
    } else {
        str = "<font size='3' color='red'>ไม่ถูกต้อง</font>";
        document.getElementById("checkQ6").innerHTML = str;
    }

    //  Question 7
    if(question7 == "mysqli_connect") {
        str = "<font size='3' color='green'>ถูกต้อง</font>";
        document.getElementById("checkQ7").innerHTML = str;
        correct7 = 2;
        } else {
            str = "<font size='3' color='red'>ไม่ถูกต้อง</font>";
            document.getElementById("checkQ7").innerHTML = str;
        }

        //  Question 8
    if(question8 == "mysqli_select_db") {
        str = "<font size='3' color='green'>ถูกต้อง</font>";
        document.getElementById("checkQ8").innerHTML = str;
        correct8 = 2;
        } else {
            str = "<font size='3' color='red'>ไม่ถูกต้อง</font>";
            document.getElementById("checkQ8").innerHTML = str;
        }

        //  Question 9
    if(question9 == "ฟังก์ชันในการส่งคืนคำอธิบายข้อผิดพลาด") {
        str = "<font size='3' color='green'>ถูกต้อง</font>";
        document.getElementById("checkQ9").innerHTML = str;
        correct9 = 2;
        } else {
            str = "<font size='3' color='red'>ไม่ถูกต้อง</font>";
            document.getElementById("checkQ9").innerHTML = str;
        }

    // Display
    var point = correct1 + correct2 + correct3 + correct4 + correct5 + correct6 + correct7 + correct8 + correct9;

    document.getElementById("after_submit").style.visibility = "visible";
    document.getElementById("number_correct").innerHTML = "คุณได้ " + point + " คะแนนจากทั้งหมด 18 คะแนน";
    document.getElementById("point").value = point;
}