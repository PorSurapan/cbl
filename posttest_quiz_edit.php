...
<head>
	<title>[QUIZ] Post-Test Edit Data</title>
	<meta charset="utf-8">
	...
	<script src="searchUserPostEdit.js"></script>
</head>
...
<body>
	...
	<div class="wrapper d-flex align-items-stretch">
		...
		<div id="content" class="p-4 p-md-5 pt-5">
			<h2 class="mb-4">แบบทดสอบหลังเรียน - การแก้ไขข้อมูล</h2>
            ...
            <table width="100%">
                <tr>
                    <form action="posttest_quiz_edit.php" method="get">
                        <td>
                            <b>ค้นหาชื่อ </b>
                            <input list="word" name="search" placeholder="กรอกคำค้น" onkeyup="searchUserPostEdit(this.value)" />
                            <datalist id="word">
							</datalist>
                    ...
                    <form name="form" action="posttest_quiz_edit.php" method="post">
                        <td>
                            <b>เรียงตาม </b>
                        ...
                        <?php
							...
							if($search == "")
							{
								if($sort == "เรียงตามผู้ลงทะเบียนล่าสุด")
									$sql = "SELECT * FROM editpost ORDER BY user_id DESC";
								else if($sort == "เรียงตามผู้ลงทะเบียนเก่าสุด")
									$sql = "SELECT * FROM editpost ORDER BY user_id ASC";
								else if($sort == "เรียงตามตัวอักษร (ชื่อผู้ใช้)")
									$sql = "SELECT * FROM editpost ORDER BY user_username";
								else if($sort == "เรียงตามตัวอักษร (ชื่อ - นามสกุล)")
									$sql = "SELECT * FROM editpost ORDER BY user_name";
								else if($sort == "เรียงตามคะแนน (มาก - น้อย)")
									$sql = "SELECT * FROM editpost ORDER BY post_test DESC";
                                else if($sort == "เรียงตามเวลา (ล่าสุด - เก่าสุด)")
									$sql = "SELECT * FROM editpost ORDER BY date_time DESC";
                                else
                                    $sql = "SELECT * FROM editpost ORDER BY pass";
							}
							else
							{
								$sql = "SELECT * FROM editpost WHERE user_username LIKE '%$search%' OR user_name LIKE '%$search%'";
							}
